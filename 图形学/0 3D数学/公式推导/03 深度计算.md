...
首先对图像空间下的视锥体射线（从摄像机出发，指向图像上某点的射线）进行插值，这条射线存储了该像素在世界空间下摄像机的方向信息。然后把该射线和**线性化后的观察空间下的深度值**相乘，再加上摄像机的世界位置，就可以得到该像素在世界空间下的位置。

我们只需要知道摄像机在世界空间下的位置，以及世界空间下该像素相对于摄像机的偏移量，把它们相加就可以得到该像素的世界坐标。用代码表示：
```
float4 worldPos = _WorldSpaceCameraPos + linearEyeDepth * interpolatedRay
```
`_WorldSpaceCameraPos` 是摄像机在世界空间下的位置
`linearEyeDepth * interpolatedRay` 可以计算得到该像素相对于摄像机的偏移量
`linearEyeDepth` 是由深度纹理得到的观察空间下的线性深度值
`interpolatedRay` 是由**顶点着色器**输出并插值后得到的射线，它不仅包含了该像素到摄像机的方向，也包含了距离信息。

`interpolatedRay` 来源于对近裁剪平面的 4 个角的某个特定向量的插值，这 4 个向量包含了它们到摄像机的方向和距离信息，我们可以利用摄像机的近裁剪平面距离 $Near$、$FOV$、横纵比 N $aspect$ 计算而得。


---

在某些情况下，我们需要**屏幕后处理阶段**得到像素点对应的世界坐标。如下图，我们在屏幕后处理阶段，想要知道屏幕空间中 A1点对应的世界坐标 A 点。那么 A 点该怎么求呢？
...
而 A 点的世界坐标 = O 点的世界坐标 + $\overrightarrow{OA}$
我们要做的就是求 $\overrightarrow{OA}$ 

主要步骤如下：
1. 当顶点为 $LT$ 点（即屏幕左上角）时将 $\overrightarrow {OLT}$ 向量的值放置在顶点着色器输出结构体中（eg: 常用的符号 v2f）；
...
![[Pasted image 20230709123140.png]]
...
————————————————
版权声明：本文为 CSDN 博主「WangShade」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
原文链接： https://blog.csdn.net/sinat_25415095/article/details/124764443
...
$$
\begin{gathered}
TL=camera.forward \cdot Near+to Top-to Right\\
...
![[Pasted image 20230709120957.png|700]]
>如图，以空间中的的A 点为例，$|OD|$ 为 A 点在观察空间的线性深度，$|OA|$ 才是欧氏距离

我们要先把线性深度值转换为欧氏距离，再和四个角的向量相乘来计算他们到摄像机的偏移量。以 $TL$ 为例，根据相似三角形原理，$TL$ 所在的射线上，像素的深度值 $eyeDepth$ 和它到摄像机的实际距离 $dist$ 的比等于近裁剪平面的距离和 $TL$ 向量的模的比，即
$$\frac{eyeDepth}{dist}=\frac{Near}{\mid TL\mid}$$
由此得到欧氏距离 $dist$ $$dist=\frac{\mid TL\mid}{Near}\times eyeDepth$$

$$
\begin{aligned}Ray_{TL}&=\frac{TL}{\mid TL\mid}\times scale,Ray_{TR}=\frac{TR}{\mid TR\mid}\times scale\\Ray_{BL}&=\frac{BL}{\mid BL\mid}\times scale,Ray_{BR}=\frac{BR}{\mid BR\mid}\times scale\end{aligned}
$$

屏幕后处理的原理是使用特定的材质去渲染一个刚好填充整个屏幕的四边形面片。这个四边形面片的 4 个顶点就对应了近裁剪平面的 4 个角。因此，我们可以把上面的计算结果传递给顶点着色器，顶点着色器根据当前的位置选择它所对应的向量，然后再将其输出，经插值后传递给片元着色器得到 `interpolatedRay`，我们就可以直接利用本节一开始提到的公式重建该像素在世界空间下的位置了。
