...
> 4. 由于 View space 和 NDC space 都是右手系，所以变换后 $z$ 轴方向并没有变化，因此 $n$ 被映射到了 $1$，而 $f$ 被映射到了 $-1$，仍然符合 $f < n$。（这个地方不如 OpenGL 直观，在 OpenGL 中，View Space 是右手坐标系，而 Clip Space 与 NDC Space 是左手坐标系，因此 $n$ 被映射到 $-1$，$f$ 被映射到 $1$）
> 5. 由于第四点的原因，ZBuffer 在近平面处为 1.0，在远平面处为 0.0
> 
...
## 线性深度
当我们想要精确表达物体的深度差异或者重建像素世界坐标位置，就需要使用线性深度。线性深度分为两种：
1. $LinearEyeDepth$：观察空间下的深度值，取值范围$[near, far]$
2. $Linear01Depth$：把线性深度归一化到 0~1，我们通常会使用这个线性深度

前面提到，线性深度即观察空间的深度 $z$
由方程（1）（2）可得：
$$ \begin{cases}z''=(\frac{n+f}{n-f})-\frac{2nf}{(n-f)z}  \\
z'' = NonLinearDepth\times2-1\end{cases}$$
联立可以求出 $z$，我们将其标记为 $LinearEyeDepth$
$$LinearEyeDepth=\frac{1}{NonLinearDepth\times \frac{{f-n}}{fn}+\frac{1}{f}}$$
我们只需要将 $LinearEyeDepth$ 除以 $far$ 即可得到归一化的线性深度 $Linear01Depth$
## 深度计算
