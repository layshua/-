...
### 滤波 Filtering 
- 滤波：去除特定的频率的内容
- 高通滤波（High-pass filter）：只保留高频信号（高频信号一般在边界）
- 低通滤波（Low-pass filter）：只保留低频信号（画面变模糊）

我们之前光栅化抗锯齿做的先模糊就是采用了低通滤波器（Low-pass filter），使其高频信号边界去除然后图形就会变得模糊。

![[487b949ab2557b6eee493568fdda5db5_MD5.webp|850]]

### 卷积

滤波也可以看做卷积（Convolution）, 而卷积采用了平均的方法。

如下图所示，对于一系列信号，**滤波器就好像一个窗口**，它可以左右移动，窗口的大小是一个 $3\times3$ 的格子，这其实是要做一个**卷积（Convolution）操作**，也就是说移动窗口的时候，窗口三个数所对应的信号的三个数做一个**点积的操作**。
