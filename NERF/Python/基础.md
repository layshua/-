...
**`get(key[, default])`**：提供了更宽松的方式去访问字典项，当键不存在的时候，get()方法并不会报错，只是默默地返回了一个None，表示啥都没找到： 
eg：
