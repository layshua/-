...
可以说 Half Lambert 是 Wrap Light 的一个特例。Wrap Light（环绕光照） 直译的话就是用光包裹住整个物体，让原来黑暗（即 $N\cdot L<=0$）的部分亮起来的意思。如果是标准 Lambert 的话，有很大一部分地方是黑的，让整个物体显得比较平，而 Wrap Light 会有一个光的过度，并且"soft“整个物体的感觉。如下图：
$$
\mathbf{c}_{diffuse}=\mathbf{c}_{light}\cdot\mathbf{m}_{diffuse}\max(0,\frac{n\cdot l + wrap}{ (1+ wrap)});

$$

Half Lambert 相当于 wrap 值为 1 的情况，具体项目中，wrap 值会按照美术的感觉来走。
