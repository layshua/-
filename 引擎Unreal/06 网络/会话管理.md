...
### Create Session[​](https://cedric-neukirchen.net/docs/session-management/sessions-in-cpp#create-session "Direct link to Create Session") 创建会话
...
这是使用 "FOnlineSessionSettings "结构体和 "FOnCreateSessionCompleteDelegate "委托所必需的。其他方法的未来委托也是如此。
...
When using “GetSessionInterface(..)”, always make sure to pass in the current UWorld. There are two versions of these helper functions.  
使用 "GetSessionInterface(...) "时，请务必输入当前的 UWorld。这些辅助函数有两个版本。  
One is declared and defined in the Online.h file, not needing a UWorld, and one is declared and defined in the OnlineSubsysbtemUtils.h file, requiring the current UWorld.  
其中一个在 Online.h 文件中声明和定义，不需要 UWorld，另一个在 OnlineSubsysbtemUtils.h 文件中声明和定义，需要当前的 UWorld。  
The difference is that the UWorld version takes into account that the Editor has multiple UWorlds at the same time.  
...
你会经常看到这种情况。有些子系统可能支持其他会话，如 PartySessions，但我们在本篇文章中不会讨论这些。它将会话类型描述为游戏（游玩）会话。
...
### Update Session[​](https://cedric-neukirchen.net/docs/session-management/sessions-in-cpp#update-session "Direct link to Update Session") 更新会议
...
### Start Session[​](https://cedric-neukirchen.net/docs/session-management/sessions-in-cpp#start-session "Direct link to Start Session") 开始会议
