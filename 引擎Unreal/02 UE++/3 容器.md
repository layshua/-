...
---
## TArray 数组
![[Pasted image 20231010200638.png]]
...
### 两种打印方式的区别
```c++
//打印字符串时两种打印方式还是有区别的：AddOnScreenDebugMessage需要使用FString::Printf
UE_LOG(LogTemp, Warning, TEXT("EffectAssetTags: %s"), *Tag.ToString());

GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, FString::Printf(TEXT("EffectAssetTags: %s"), *Tag.ToString()));
```

### FString:: Format 和 FString:: Printf
FString:: Format 和 FString:: Printf 都是用于格式化字符串的函数，但它们有一些区别。

1. 参数传递方式：
    - FString::Format：参数以TArray的形式传递，每个参数都需要使用FStringFormatArg结构进行封装。
    - FString::Printf：参数以可变参数列表的形式传递，可以直接传递多个参数。
2. 格式化字符串语法：
    - FString::Format：使用"{0}", "{1}", "{2}"等占位符来表示参数的位置。
    - FString::Printf：使用"%s", "%d", "%f"等格式化符号来表示参数的类型和位置。
3. 返回值：
    - FString::Format：返回格式化后的字符串。
    - FString::Printf：返回void，直接将格式化后的字符串输出到指定的输出流中（如标准输出流）。

示例代码如下：

```cpp
cpp// 使用FString::Format进行字符串格式化
FString name = "John";
int age = 25;
FString formattedString = FString::Format(TEXT("My name is {0} and I am {1} years old."), {FStringFormatArg(name), FStringFormatArg(age)});

// 使用FString::Printf进行字符串格式化
FString formattedString = FString::Printf(TEXT("My name is %s and I am %d years old."), *name, age);
```

总结：FString::Format更加灵活，可以在格式化字符串中指定参数的位置，而FString::Printf更加简洁，直接使用格式化符号表示参数的类型和位置。
