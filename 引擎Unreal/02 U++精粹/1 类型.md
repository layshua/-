...
> 因为 FText 提供了内置的本地化支持，也通过一张查找表来支持运行时本地化。
> FText **不提供任何的更改操作**。
...
```c++ file:FName/FText
//FName 转 FText 
MyText = FText::FromName(MyName); //不会自动本地化
...
`FText` 和 `Fstring` 间可相互转换。

**`FText` 转 `FString`**：有损失，将废弃（或无法创建）本地化数据。

**转换函数**：可生成**不含本地化数据**的 `FText` 字符串：

| `FText` 函数|描述|
|---|---|
|`AsCultureInvariant`|以现有 `FString` 创建非本地化且语言不变的 `FText` 实例。|
|`FromString`|通过现有的 `FString` 创建一个非本地化的 `Ftext` 实例。<br><br>此效果等同于非编辑器版本中的 `AsCultureInvariant`。在编辑器版本中，此函数不会将文本标记为语言不变，也就是说若将其指定到已保存资源中的 `FText` 属性，其仍为可本地化状态。|
