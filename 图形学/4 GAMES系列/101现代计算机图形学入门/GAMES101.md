...
可将经过常数插值或线性插值的纹理定义为一个返回向量值的函数 $T (u, v) =(r, g, b, a)$，即给定纹理坐标 $(u, v)∈[0, 1]$，则上述纹理函数 $T$ 将返回颜色 $(r, g, b, a)$。Direct3D 允许我们采用下列 4 种不同方式（即寻址模式，address mode)来扩充此函数的定义域（解决输入值超出定义域 $[0, 1]$ 这一问题)：
- 重复寻址模式 ( wrap)：通过在坐标的每个整数点处重复绘制图像来拓充纹理函数。![[Pasted image 20230520151724.png|400]]

- 边框颜色寻址模式 ( border color，也有译作边界颜色寻址模式)：通过将每个不在范围 $[0,1]$ 内的坐标 $(u, v)$都映射为程序员指定的颜色而拓充纹理函数（见图 9.11 )。
