...
*   [1. torch.utils.data.Dataset](#1_torchutilsdataDataset_3)
*   [2. torch.utils.data.DataLoader](#2_torchutilsdataDataLoader_89)
*   [3. transforms](#3_transforms_127)
*   [3.1 pytorch 官方 API](#31_pytorchAPI_128)
*   *   *   [3.1.1 transforms.Compose](#311_transformsCompose_179)
        *   [3.1.2 transforms.ToTensor](#312_transformsToTensor_228)
    *   [3.2 自定义 transforms](#32_transforms_260)

在 [pytorch](https://so.csdn.net/so/search?q=pytorch&spm=1001.2101.3001.7020) 中，提供了一种十分方便的数据读取机制，即，使用`torch.utils.data.Dataset`与`torch.utils.data.DataLoader`组合得到数据迭代器。在每次训练时，利用这个迭代器输出每一个 batch 数据，并能在输出时对数据进行相应的预处理或[数据增强](https://so.csdn.net/so/search?q=%E6%95%B0%E6%8D%AE%E5%A2%9E%E5%BC%BA&spm=1001.2101.3001.7020)等操作。

# 1. [torch](https://so.csdn.net/so/search?q=torch&spm=1001.2101.3001.7020).utils.data.Dataset
