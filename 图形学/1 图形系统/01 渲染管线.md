...
几何处理阶段可以细分为 4 个子阶段：**顶点着色器阶段 (Vertex Shading)**，**投影阶段 (Projection)**，**裁剪阶段 (Clipping)** 和**屏幕映射阶段 (Screen Mapping)**。
...
完成顶点着色阶段的两个主要任务后，在 GPU 上可以按照以下顺序进行几个**可选阶段**：**曲面细分阶段**（tessellation Stage），**几何着色阶段**（geometry shading）和**流输出**（stream output）。
...
1. **外壳着色器阶段 Hull Shader（HS） ：细分为两个着色器**
    - 常量外壳着色器：对每个**面片（Patch）** 逐一处理，输出网格的曲面细分因子和其他面片信息，曲面细分因子指示了在曲面细分阶段中将面片细分后的份数。
        - 优化细节：如果将所有的曲面细分因子设置为 0，则该面片会被后续的处理阶段丢弃，我们可以利用这个特性以每个面片为基准实现视锥体剔除和背面剔除这类优化。
        -  ![[Pasted image 20230628102005.png|500]]
    - 控制点外壳着色器：以控制点作为输入和输出，改变曲面。比如将普通的三角形（3 个控制点）转换为 3 次贝塞尔三角形面片（10 个控制点）
2. **镶嵌器阶段 Tessellator Stage**：不可控制，由硬件处理，基于外壳着色器输出的曲面细分因子，对面片进行细分操作。并输出所有新建的顶点和三角形
3. **域着色器阶段 Domain Shader（DS）** ：对镶嵌器传来的细分后的顶点进行处理，此时顶点处于面片域空间（patch domain space）。用户可以控制是否将处理后的顶点转换到齐次裁剪空间。（龙书：随着曲面细分功能的开启，顶点着色器便化身为“处理每个控制点的顶点着色器”，而外壳着色器"的本质实为“针对已经过镶嵌化的面片进行处理的顶点着色器”）
...
-   称为 Patch（面片），可以看成是多个顶点的集合，包含每个顶点的属性。**（属性是所有顶点共享的，不是每个顶点有独自的属性）**
