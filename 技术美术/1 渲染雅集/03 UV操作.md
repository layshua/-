...
```c
float2 screenUV = (input.positionoCS.xy/_ScreenParams.xy);
```
通过除以 `_ScreenParams.xy` 获取屏幕 UV 会受到 RenderScale 影响，有没有办法不受影响？
1. 手动计算
```c
//在顶点着色器里，计算屏幕 uv，并把 xy 存储为未经透除的 uv 通道，zw 正常存储。  
o.screenPos.xy = o.positionoCS.xy * 0.5+ 0.5 * float2(o.positionoCS.w,o.positionoCS.w);  
o.screenPos.zw = o.positionoCS.zw;


//在片元着色器里进行透视除法，在检测一下平台矫正 uv
i.screenPos.xy *= rcp(i.screenPos.w); //透视除法，rcp取近似倒数  
//i.screenPos.xy/=i.screenPos.w; //透视除法  
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float2 screenUV = (input.positionoCS.xy/_ScreenParams.xy)+float2(distortX,distortY);
