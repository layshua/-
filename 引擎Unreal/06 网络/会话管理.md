...
## Creating Sessions[​](https://cedric-neukirchen.net/docs/session-management/#creating-sessions "Direct link to Creating Sessions") 创建会话

For Players to find a Session and potentially join it, you need to create a Session and set up its properties, as well as decide which of those properties will be visible. Registering a Dedicated Server Session via Blueprint might not be supported in the current Engine Version.  
要让玩家找到会话并加入会话，需要创建一个会话并设置其属性，以及决定哪些属性是可见的。当前的引擎版本可能不支持通过蓝图注册专用服务器会话。  
In C++ you should do the normal C++ Session creation inside the AGameSession by overriding the 'RegisterServer' function!  
在 C++ 中，您应该通过覆盖 "RegisterServer "函数，在 AGameSession 内部创建正常的 C++ 会话！

### Creating a Session via Blueprint[​](https://cedric-neukirchen.net/docs/session-management/#creating-a-session-via-blueprint "Direct link to Creating a Session via Blueprint")  
通过蓝图创建会话
...
## Updating Sessions[​](https://cedric-neukirchen.net/docs/session-management/#updating-sessions "Direct link to Updating Sessions") 更新会话
...
更新会话的请求完成后，"OnUpdateSessionComplete "委托将被触发。这样就有机会执行处理会话设置更改所需的任何配置或初始化。
