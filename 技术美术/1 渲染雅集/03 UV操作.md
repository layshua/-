...
2. （推荐）顶点着色器计算的顶点齐次裁剪坐标 xy 分量就是屏幕空间的物理像素大小，利用它除以屏幕分辨率 `_ScreenParams.xy` 即可
```c
...
4. **用像素位置除以渲染目标分辨率 `_ScaledScreenParams`**。`_ScaledScreenParams.xy` 属性会考虑渲染目标的任何缩放，例如动态分辨率。
```c 
