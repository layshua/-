...
第一步：在 C# 中设置 Camera.main.depthTextureMode = DepthTextureMode.Depth;

可以在主摄像机的 Camera 组件下看见提示：

![[8c9f6e1c9694558c2fe89b8061c12aab_MD5.webp]]

这表明了主摄像机渲染了深度图

第二步：在 Shader 中声明_CameraDepthTexture
...
利用覆盖屏幕的 uv 值和深度图中的深度，我们可以重建出物体在世界空间中的坐标。  
主要有以下两种方法：

1.  利用 VP 矩阵的逆矩阵对 NDC 坐标进行转换。
2.  找到从摄像机指向该点的方向向量（需要是单位向量），将该方向向量乘上深度值就能得到摄像机指向该点的向量，将该向量加上摄像机位置就能得到该点的世界坐标。

### 1. 利用 VP 矩阵重建

首先要在 C# 脚本中传递当前的 VP 逆矩阵：
...
具体用法可以到下面的 MotionBlur 例子中查看。

### 2. 利用方向向量重建

首先需要知道，Post Process 实际上是渲染一个覆盖屏幕的 Quad，因此屏幕四个角对应摄像机的视椎体四个角。  
首先是算出摄像机到四个角的向量：

![[6dc844e681c24976f93aadf5142f2b06_MD5.webp]]
...
![[149d53e44310cbb74369f1ae3dc426a1_MD5.webp]]
...
![[52ac6d06b4e77e42bde26c124f8e3443_MD5.webp]]

假设有个绿点在 toTopLeft 所在线上，利用相似三角形，可以得到：

```
toGreen / depth = toTopLeft / near
```

而 depth 是能够在 Shader 中获得的，因此我们只需要传递 toTopLeft / near 到 Shader 中就能计算出 toGreen：
...
在 Vertex 中判断出对应顶点所在的向量：

![[92565f339916c8ea76b213ed71d2fb20_MD5.webp]]

可以看到 uv 值和对应的索引值正好是二进制的关系，所以可以如下求出：
...
你可能奇怪这样只能求到 4 个角线上的点，但 vertex 到 fragment 的过程中是有个东西叫插值的，这个**插值**正好能把每个像素所在的向量求出。  
然后我们就能在 fragment 中求出世界坐标了：
...
[完整代码点这里](https://link.jianshu.com?t=https%3A%2F%2Fgithub.com%2FKaimaChen%2FUnity-Shader-Demo%2Fblob%2Fmaster%2FUnityShaderProject%2FAssets%2FDepth%2FShaders%2FPrintDepth.shader)

![[234dc5b372869faf85c9ec1047845c63_MD5.webp]]

PrintDepthMap 场景

PS: 如果代码没错，而看到的是全黑的，那么应该就是摄像机的 Far Clip Plane 设得太大。
