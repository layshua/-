...
> Unity 从裁剪空间到屏幕空间的转换是由引擎完成的，我们在顶点着色器中只需要把顶点转换到裁剪空间。（然后当片元着色器使用裁剪空间位置 `i.positionCS` 时，xy值等于屏幕空间坐标xy，除以屏幕分辨率及可以得到归一化的屏幕坐标 (即屏幕 UV)）
**重要结论：**
裁剪空间 w 值等于观察空间 z 值，即深度。
裁剪空间之后，zw 值都不变。
...
- **屏幕空间**
    ...
    - `z、w`：**Screenpos.zw == posCS.zw**，具体查看ComputeScreenPos()方法
    - 通常z分量会被用于深度缓冲。 

- **摄像机视口空间**：![[Pasted image 20230801151519.png|500]]
- `x[0, 1]`：归一化后的屏幕X坐标，左下角为0，右上角为1。
- `y[0, 1]`：归一化后的屏幕 Y 坐标，左下角为0，右上角为1。

> i.positionCS. xy 是像素屏幕坐标位置 （注意和 0-1 屏幕 UV 的区别）
> i.positionCS. zw = ScreenUV. zw
>  z 是像素的深度
>  w=1
...
- @ 方法一：使用 RenderFeature 传递 RT。设置 Render Pass 为 EventAfter Rendering Post Processing
...
m_blitMaterial.SetTexture("_CustomTex", m_tempRT0); //临时RT传入材质
//或应用于全部shader
cmd.SetGlobalTexture("_CustomTex", m_tempRT0); 
```
