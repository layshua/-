...
# 屏幕空间 UV
## 获取屏幕空间 UV
1. 使用 `VPOS` 语义获取屏幕空间 uv：[[1 ShaderLab#VPOS 屏幕空间像素位置]]
2. 顶点着色器计算的顶点齐次裁剪坐标 xy 分量就是屏幕空间的物理像素大小，利用它除以屏幕分辨率 `_ScreenParams.xy` 即可。
...
o.uvSS.xy = o.positionCS.xy*0.5 + 0.5* float2(o.positionCS.w,o.positionCS.w);  
o.uvSS.zw = o.positionCS.zw;


//在片元着色器里进行透视除法，在检测一下平台矫正 uv
i.uvSS.xy *= rcp(i.uvSS.w); //透视除法，rcp取近似倒数  
//i.uvSS.xy/=i.uvSS.w; //透视除法  
...
4. **用像素位置除以渲染目标分辨率 `_ScaledScreenParams`**。`_ScaledScreenParams.xy` 属性会考虑渲染目标的任何缩放，例如动态分辨率。分量含义和 `_ScreenParams` 一样
```c 
...
### CameraOpaqueTexture
URP Asset 开启开启 Opaque texture，降采样用于调整抓屏的分辨率
OpaqueTexture 只能渲染不透明物体。
...
//使用屏幕uv对其采样
float2 uvSS = (input.positionCS.xy/_ScreenParams.xy);  
float4 ScreenColor = tex2D(_CameraOpaqueTexture,uvSS);
```
...
SubShader
{
    ...
    Pass
    {
        ...
        float4 frag(Varyings input) : SV_Target  
        {
            float2 uvSS = (input.positionCS.xy/_ScreenParams.xy);  //屏幕坐标
            float4 finalColor = tex2D(_AfterPostProcessTexture,uvSS); //采样
        }
}
```
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float2 uvSS = (input.positionCS.xy/_ScreenParams.xy)+float2(distortX,distortY);
                
                float4 finalColor = tex2D(_AfterPostProcessTexture,uvSS);
                //mask控制透明度
...
### (未验证)修改 URP
**支持不透明抓屏：可以考虑在 renderfeature 中在 Transparent 渲染完后，将屏幕图像手动 Blit 到一张 RT 中，作为参数传给扭曲 shader，用作 OpaqueTexture 的替代。
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings i) : SV_Target
            {
                ...
                float2 uvSS = i.positionCS.xy / _ScreenParams.xy ;
                //采样时加上偏移
                float4 cameraColor = tex2D(_CameraOpaqueTexture,uvSS+bias);
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float4 finalColor = tex2D(_CameraOpaqueTexture,uvSS);
                //mask控制透明度
                float mask = SAMPLE_TEXTURE2D(_MaskTex,sampler_MaskTex,input.uv.zw).r;
                return float4(finalColor.xyz,mask);
            }
