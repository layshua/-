...
模板参数的命名应当遵循对应的分类: 类型模板参数应当遵循 [类型命名](#type-names) 的规则, 而非类型模板应当遵循 [变量命名](#variable-names) 的规则.
...
枚举的命名应当和 [常量](#constant-names) 或 [宏](#macro-names) 一致: `kEnumName` 或是 `ENUM_NAME`.
...
2009 年 1 月之前, 我们一直建议采用 [宏](#macro-names) 的方式命名枚举值. 由于枚举值和宏之间的命名冲突, 直接导致了很多问题. 由此, 这里改为优先选择常量风格的命名方式. 新代码应该尽可能优先使用常量风格. 但是老代码没必要切换到常量风格, 除非宏风格确实会产生编译期问题.
