...
**其他逐像素的灯光会在额外的 Pass 中渲染，每一个灯光会产生一个额外的 Pass。在额外 Pass 中的灯光默认不会投射阴影。这意味着默认情况下，前向渲染只支持一个投射阴影的平行光**。
如果希望更多的灯光能够产生投影，就需要添加内置的 `multi_compile_fwdadd_fullshadows` 编译指令编译出不同的 Shader 变体（Variant）。
