...
c_i=\int_{\Omega}B_i(\omega)B_i(\omega)\mathrm{~d}\omega =0
$$ 简单地投影 / 重建、simple rotation。**
...
2. 而 light transport (visibility 和 brdf) 是不变的, 因此相当于对任一 shading point 来说, light transport 项固定的, 可以认为是 shading point 自己的性质, light transport 总体来说还是一个球面函数, 因此也可以写成基函数形式, 是可以预计算出的。
...
2. 对于预计算的光源我们把它投影到 sh 上, 如果光源发生了旋转, 那不就相当于换了个光源吗?这个问题由于 sh 函数的旋转不变性可以完美的解决.**
    - **旋转光照 = 旋转 SH 的基函数**
