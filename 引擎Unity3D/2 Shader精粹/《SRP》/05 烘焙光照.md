...
1. 就像采样光照贴图一样，我们要使用光照贴图的 UV 坐标。不同的是，**我们将 UV 赋值给对象空间顶点位置的 XY 分量，Z 分量需要限制到 `[0，FLT_MIN]` 区间，其中 FLT_MIN 代表最小的正浮点数**。然后使用 TransformWorldToHClip 方法将该顶点位置转换到裁剪空间中（尽管传入的不是世界空间的顶点位置，不过还是需要这么做）。
