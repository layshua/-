...
```c
float2 uvSS = (input.positionoCS.xy/_ScreenParams.xy);
```
3. 手动计算
...
**不透明物体方法：**
1. Overlay的Camera，继承MainCamera的画面通过RenderFeature就能进行扭曲效果的处理。（URP2022 失效失效？）：[URP | 热扭曲效果 - 哔哩哔哩 (bilibili.com)](https://www.bilibili.com/read/cv15512651)
2. (未验证)修改 URP 的代码，我们只需要在 PostProcessPass 里，DrawMesh 的时候，判断当前是否有需要扭曲效果的物体，如果有，就需要创建一个新的 RT：
```cs
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float2 uvSS = (input.positionoCS.xy/_ScreenParams.xy)+float2(distortX,distortY);
