...
完成顶点着色阶段的两个主要任务后，在 GPU 上可以按照以下顺序进行几个**可选阶段**：**曲面细分阶段**（tessellation），**几何着色阶段**（geometry shading）和**流输出**（stream output）。
并不是所有 GPU 都支持它们，它们彼此独立，并且一般来说不常用
### 曲面细分阶段
Hull Shader（HS） 外壳着色器阶段
Tessellator Stage 曲面细分阶段
Domain Shader（DS） 域着色器阶段
