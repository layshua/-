...
我们在模水面时，如果水面的起伏不是非常明显，直接使用法线纹理动画就可以得到不错的结果 (在《UnityShader 入门精要》15.2 节, 有详细的讲解)。如果想要一些比较明显的波形，我们可以使用最简单的波形 - 正弦波，我们来看下最简单的公式形式
...
我们调节振幅 A 的值，可以看到振幅就是从水平面到波峰的高度。然后加入第二个参数角频率 $\omega$ ，公式为
...
Gerstner 波是将水平位置进行挤压，使得波峰变尖，波谷变宽。下面简单演示了 Gerstner 是如何进行挤压的。原本是打算也在 Desmos 上画图形的，但是不太清楚怎么控制离散的点，只能用 Excel 来拯救我了 ==
